...
p = -np.ones((Nx, Ny))
r = 0.15 * Lx  # radius
d = r / np.sqrt(2)  # shift along diagonal so they touch at center
c1 = (Lx/2 - d, Ly/2 - d)
c2 = (Lx/2 + d, Ly/2 + d)
dist1 = (X - c1[0])**2 + (Y - c1[1])**2
dist2 = (X - c2[0])**2 + (Y - c2[1])**2
p[(dist1 <= r**2) | (dist2 <= r**2)] = 1
...
anim.save("media/videos/Cahn_Hilliard_2D_water_drops.mp4", writer=writer)
print("Animation saved as media/videos/Cahn_Hilliard_2D_water_drops.mp4")
